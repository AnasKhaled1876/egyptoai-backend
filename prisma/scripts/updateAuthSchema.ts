...
async function main() {
  ...
  try {
    ...
    const usersWithNullPassword = await prisma.user.findMany({
      where: { hashed_password: null },
    });

    if (usersWithNullPassword.length > 0) {
      console.log(`Found ${usersWithNullPassword.length} users with null passwords`);
      // Set a secure random password that won't be usable for login
      ...
      await prisma.user.updateMany({
        ...
        data: { 
          ...
          // Mark these users as needing password reset
          salt: 'reset_required',
        },
      });
      console.log('Updated users with null passwords');
    } else {
